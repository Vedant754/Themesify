...
let audio = new Audio(`media/songs/${random + 1}.mp3`);
console.log(random);

// Show social links on owner click
...
// Display result when guess is submitted
const correctSong = songs[random].answer;
...
opt[0].innerText = songs[random].option1;
opt[1].innerText = songs[random].option2;
opt[2].innerText = songs[random].option3;
opt[3].innerText = songs[random].option4;

// Randomize correct answer button
let r = Math.floor(Math.random() * 4);
opt[r].innerText = songs[random].answer;
...
function play() {
    audio.play();
    question.innerText = songs[random].question;
